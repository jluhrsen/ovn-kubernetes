...
// Provider is the standalone SSH infra provider for clusters whose nodes are
// containers on a single container runtime reachable over SSH (the upstream
// kind-over-ssh guardrail). It drives external-container/network work through
// container.Engine and reaches node commands through a pluggable NodeExecutor.
...
func New(cfg Config) (*Provider, error) {
	...
	if sshRunner == nil {
		return nil, fmt.Errorf("ssh: Provider requires a non-nil runner")
	}
	p := &Provider{
		cfg:      cfg,
		runner:   sshRunner,
	...
	if nodeExec == nil {
		switch cfg.NodeExecutorMode {
		case "", "container":
			nodeExec = NewEngineNodeExecutor(p.engine)
		default:
			return nil, fmt.Errorf("unsupported %s %q (supported: container)", EnvNodeExecutor, cfg.NodeExecutorMode)
		}
	}
	p.nodeExec = nodeExec
...
func (p *Provider) PrimaryNetwork() (api.Network, error) {
	return p.engine.GetNetwork(p.cfg.PrimaryNetwork)
}
...
func (p *Provider) GetK8NodeNetworkInterface(instance string, network api.Network) (api.NetworkInterface, error) {
	...
	ni, err := p.engine.GetNetworkInterface(instance, network.Name())
	if err != nil {
...
func (p *Provider) ShutdownNode(nodeName string) error {
	if err := p.engine.StopContainer(nodeName); err != nil {
...
func (p *Provider) StartNode(nodeName string) error {
	if err := p.engine.StartContainer(nodeName); err != nil {
...
func (p *Provider) ListNetworks() ([]string, error) {
	return p.engine.ListNetworks()
}

func (p *Provider) GetNetwork(name string) (api.Network, error) {
	return p.engine.GetNetwork(name)
}
...
// delegated to a context-bound container engine so their cleanup is registered
// on the test context.
